		...
		for(int y=0; y<reduced_YDIM; y++){
			for(int x=0; x<reduced_XDIM; x++){
		        ...
		        for(int z=0; z<target_z; z++){
		        	for(int y=0; y<target_y; y++){
		        		for(int x=0; x<target_x; x++){
		        			float px = gridMap[z*target_y*target_x*3 + y*target_x*3 + x*3 + 0];
		        			float py = gridMap[z*target_y*target_x*3 + y*target_x*3 + x*3 + 1];
		        			float pz = gridMap[z*target_y*target_x*3 + y*target_x*3 + x*3 + 2];

		        			if(isInside(px,py,pz,new_xmin,new_xmax,new_ymin,new_ymax,new_zmin,new_zmax)){
		        				//count_points++;
		        				int grid_id = z*target_y*target_x + y*target_x + x;
