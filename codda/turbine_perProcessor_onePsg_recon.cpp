...
int main(int argc, char* argv[])
{
	...
	string full_vts_file = "/media/hazarika/WD2/ResearchDataStore2/codda/turbine/turbine_single_passage_new/full_grid_data/time1/out_"+procId+"_1.vts";
	...
	string target_vts_file = "/media/hazarika/WD2/ResearchDataStore2/codda/turbine/turbine_single_passage_new/grid_data/time15/out_"+procId+"_15.vts";
	//string target_vts_file = "/media/hazarika/WD2/ResearchDataStore2/codda/turbine/turbine_single_passage_new/full_grid_data/time1/out_"+procId+"_1.vts";
	...
	distr_filename[0] = "/media/hazarika/WD2/ResearchDataStore2/codda/turbine/turbine_single_passage_new/distrFiles/time"+timeId+"/allvar_GMM3.single_mvedda_"+procId+"_" + timeId;
	...
	para_filename[0] = "/media/hazarika/WD2/ResearchDataStore2/codda/turbine/turbine_single_passage_new/distrFiles/time"+timeId+"/allvar_corr.spcc_"+procId+"_" + timeId;
	...
	float *entropy_recon_array = new float[target_x*target_y*target_z];
	float *uvel_recon_array = new float[target_x*target_y*target_z];
	float *temperature_recon_array = new float[target_x*target_y*target_z];
	float *sumOfWeightsField = new float[target_x*target_y*target_z];

	for(int i=0; i<target_x*target_y*target_z; i++)
	{
		entropy_recon_array[i] = 0.0;
		uvel_recon_array[i] = 0.0;
		temperature_recon_array[i] = 0.0;
		sumOfWeightsField[i] = 0.0;		
	}
	...
	for(int i=0; i<target_x*target_y*target_z; i++)
	{
		...
		entropy_recon_array[i] /= sumOfWeightsField[i];
		uvel_recon_array[i] /= sumOfWeightsField[i];
		temperature_recon_array[i] /= sumOfWeightsField[i];

		
	}

	vtkSmartPointer<vtkFloatArray> reconEnt = vtkSmartPointer<vtkFloatArray>::New();
	reconEnt->SetName("recon_entropy");
	reconEnt->SetNumberOfComponents(1);
	reconEnt->SetNumberOfValues(target_x*target_y*target_z);

	vtkSmartPointer<vtkFloatArray> reconUvel = vtkSmartPointer<vtkFloatArray>::New();
	reconUvel->SetName("recon_uvel");
	reconUvel->SetNumberOfComponents(1);
	reconUvel->SetNumberOfValues(target_x*target_y*target_z);

	vtkSmartPointer<vtkFloatArray> reconTemp = vtkSmartPointer<vtkFloatArray>::New();
	reconTemp->SetName("recon_temperature");
	reconTemp->SetNumberOfComponents(1);
	reconTemp->SetNumberOfValues(target_x*target_y*target_z);

	for(vtkIdType i=0; i<target_x*target_y*target_z; i++){
	  float f1 = entropy_recon_array[int(i)];
	  reconEnt->SetValue(i,f1);

	  float f2 = uvel_recon_array[int(i)];
	  reconUvel->SetValue(i,f2);

	  float f3 = temperature_recon_array[int(i)];
	  reconTemp->SetValue(i,f3);
	}

	target_sgrid->GetPointData()->AddArray(reconEnt);
	target_sgrid->GetPointData()->AddArray(reconUvel);
	target_sgrid->GetPointData()->AddArray(reconTemp);


	delete[] entropy_recon_array;
	delete[] uvel_recon_array;
	delete[] temperature_recon_array;
	delete[] sumOfWeightsField;

	string vts_output_path = "/media/hazarika/WD2/ResearchDataStore2/codda/turbine/turbine_single_passage_new/grid_data_output/time"+timeId+"/out_"+procId+"_"+timeId+"_recon.vts";
	//string vts_output_path = "/media/hazarika/WD2/ResearchDataStore2/codda/turbine/turbine_single_passage_new/full_grid_data_output/time"+timeId+"/out_"+procId+"_"+timeId+"_recon.vts";
